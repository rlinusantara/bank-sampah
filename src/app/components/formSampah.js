...
const FormInputSampah = () => {
  ...
  const pilihan = useRef()

  const sendData = async (e) => {
    e.preventDefault();
    try {
      const kirim = await axios.post("/api/setoran-masuk", {
        id_nasabah: _id,
  ...
  return (
    <>
      ...
      {popUp ? (
        <section className="border fixed w-full flex justify-center top-60 p-5">
          <div className="flex flex-col items-center justify-center bg-background w-96 p-10 rounded-lg">
            <div>
              <CircleCheckBig size={80} />
            </div>
            <div className="p-4">
              <h2 className="text-xl">Data Berhasil Di Input</h2>
            </div>
            <button
